...
class InvestmentsView(APIView):
    ...
    def post(self, request: Request):
        ...
        if request.data["value"] > (idea[0].value - idea[0].amount_collected):
            return Response(
                {"error": f"Your investment can't be bigger than {idea[0].value-idea[0].amount_collected}"},
                HTTP_422_UNPROCESSABLE_ENTITY,
            )
        ...
        investment = Investment.objects.create(
            value=request.data["value"], percentage=pr, user_id=request.user.uuid, idea_id=request.data["idea_id"]
        )
